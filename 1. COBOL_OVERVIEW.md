...
Biên dịch file trên ra file run trùng tên, sẽ có một số warning nhưng ko có lỗi nên vẫn tạo file `helloworld` thành công

```
cobc -free -x -o helloworld helloworld.cbl

helloworld.cbl:6: warning: line not terminated by a newline
```

Chạy file `helloworld` và ta sẽ có kết quả
...
| **73 – 80**   | **Vùng nhận dạng (Identification Area)**                                                          | Dùng ngày xưa để **ghi thông tin nhận dạng** (ví dụ số file, mã lập trình viên, v.v.) khi chương trình được lưu trên thẻ giấy. <br>_Hiện nay không còn cần thiết_                                                                           |
...
- Statement: Là một lệnh nhỏ lẻ để thực hiện một hành động (vd: `MOVE`, `READ`, `DISPLAY`,...).

- Sentence: Là một hoặc nhiều câu lệnh COBOL kết thúc bằng một dấu chấm.

- Paragraph: Là đơn vị logic nhỏ nhất và cơ bản nhất, bao gồm một hoặc nhiều Sentence.

- Section: Là một nhóm các paragraph liên quan.
...
000-MAIN-LOGIC.
    ...
    STOP RUN.
*
005-PRT-EMP-DATA SECTION.                           <-- Section
* Section to print employee name.
010-MOVE-DATA.                                      <-- Paragraph
    MOVE 'ROGGER' TO WS-FNAME.
    MOVE 'MURPHY' TO WS-LNAME.
010-EXIT.
    EXIT.
*
020-PRT-DATA.                                       <-- Paragraph
    DISPLAY 'NAME OF EMPLOYEE : ', WS-EMP-NAME.     <-- Sentence
020-EXIT.
    EXIT.
```
